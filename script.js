...
const faElem = document.querySelector(".boxicons-list");
...
const renderResult = (query) => {
	...
	query_count.innerText = res.length == boxicons.length ? `All ${res.length}` : `${res.length} Resulting`
	faElem.innerHTML = res.join('');
}

const renderResultDebounce = debounce(renderResult, 500)

query_count.innerText = `All ${boxicons.length}`
